...
export async function planningIntake(repoDir, objectApiName, requirementsPath) {
    ...
    const pick = (re) => Array.from(body.matchAll(re)).map(m => (m[1] || m[0]).trim());
    ...
    const intake = { objectApiName, goals, constraints, requirements };
    const outPath = path.join(outDir, 'intake.json');
    await writeJson(outPath, intake);
    return { intakePath: outPath, sources };
}
export async function planningPropose(repoDir, objectApiName) {
    ...
    const obj = (await exists(objPath)) ? await readJson(objPath) : { objectApiName, label: objectApiName };
    const usage = (await exists(usagePath)) ? await readJson(usagePath) : { apex: [], flows: [], lwcs: [] };
    const perms = (await exists(permsPath)) ? await readJson(permsPath) : { crud: {}, fieldPermissions: {} };
    // Load intake if exists
    const intakePath = path.join(repoDir, '.artifacts', 'planning', objectApiName, 'intake.json');
    const intake = (await exists(intakePath)) ? await readJson(intakePath) : { objectApiName, goals: [], constraints: [], requirements: [] };
    const impactedApex = (usage.apex || []).map((a) => a.name).filter((v) => !!v).slice(0, 25);
...
export async function planningTicketize(repoDir, objectApiName) {
    const v2Path = path.join(repoDir, '.artifacts', 'plan', `${objectApiName}.plan.v2.json`);
    const plan = await readJson(v2Path);
    const outDir = path.join(repoDir, 'tickets', 'derived');
    await ensureDir(outDir);
    const written = [];
    (plan.tickets || []).forEach((t, idx) => {
...
export async function planningVerify(repoDir, objectApiName) {
    ...
    const result = { ok: true, mismatches: [], details: {} };
    try {
        const plan = await readJson(planV2Path);
        ...
        if (!obj.fields || obj.fields.length === 0) {
            result.ok = false;
            result.mismatches.push('No fields found in discovery object.json');
        }
        if (JSON.stringify((plan.policy?.crud) || {}) !== JSON.stringify((perms.crud) || {})) {
            result.ok = false;
            result.mismatches.push('Plan policy.crud differs from discovery permissions.crud');
        }
        // required fields must be readable; report any missing FLS
        const req = (obj.fields || []).filter((f) => f.required);
        const missingFls = req.filter((f) => !(perms.fieldPermissions || {})[f.fullName]?.readable);
        if (missingFls.length) {
            result.ok = false;
            result.mismatches.push(`Required fields without readable FLS: ${missingFls.map((f) => f.fullName).join(', ')}`);
        }
        result.details = {
        ...
        result.ok = false;
        result.mismatches.push('Missing plan or discovery artifacts');
    }
