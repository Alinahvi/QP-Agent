...
import { ensureDir, writeJson, readJson, writeText, readText } from '../lib/fsx.js';
...
async function listToolsViaCli(serverRoot) {
    const distServer = path.join(serverRoot, 'dist', 'server.js');
    const existsDist = await fse.pathExists(distServer);
    if (!existsDist) {
...
function renderToolBullets(tools) {
    const lines = [];
...
function injectSection(markId, title, body, existing) {
...
export async function repoToolsCatalog(repoDir, writeFile = true) {
    fs.appendFileSync('debug.log', 'repoToolsCatalog: start\n');
    const serverRoot = await findProjectRoot(process.cwd());
    fs.appendFileSync('debug.log', 'repoToolsCatalog: after findProjectRoot\n');
    const tools = await listToolsViaCli(serverRoot);
    fs.appendFileSync('debug.log', 'repoToolsCatalog: after listToolsViaCli\n');
    const cfg = await resolveProject(repoDir);
    fs.appendFileSync('debug.log', 'repoToolsCatalog: after resolveProject\n');
    const outDir = path.join(repoDir, cfg.artifactsDir || '.artifacts', 'catalog');
    const outPath = path.join(outDir, 'tools.json');
    if (writeFile) {
        fs.appendFileSync('debug.log', 'repoToolsCatalog: before ensureDir\n');
        await ensureDir(outDir);
        fs.appendFileSync('debug.log', 'repoToolsCatalog: after ensureDir\n');
        await writeJson(outPath, { generatedAt: new Date().toISOString(), count: tools.length, tools });
        fs.appendFileSync('debug.log', 'repoToolsCatalog: after writeJson\n');
    }
    fs.appendFileSync('debug.log', 'repoToolsCatalog: end\n');
...
export async function repoSyncToolDocs(args) {
    ...
    let readmeSrc = '';
    try {
        readmeSrc = await readText(readmePath);
    }
    catch { }
    const readmeUpdated = injectSection(markId, sectionTitle, bullets, readmeSrc);
    if (!dryRun && readmeUpdated.changed)
        await writeText(readmePath, readmeUpdated.updated);
    updates.push({ file: readmePath, changed: readmeUpdated.changed });
    // AGENTS.md
    let agentsSrc = '';
    try {
        agentsSrc = await readText(agentsPath);
    }
    catch { }
    const agentsUpdated = injectSection(markId, sectionTitle, bullets, agentsSrc);
    if (!dryRun && agentsUpdated.changed)
        await writeText(agentsPath, agentsUpdated.updated);
    updates.push({ file: agentsPath, changed: agentsUpdated.changed });
    // .mcp.project.json
    let cfg = {};
    try {
        cfg = await readJson(mcpConfigPath);
    }
    catch {
        cfg = {};
    }
    ...
    if (!cfg.artifactsDir)
        cfg.artifactsDir = (await resolveProject(repoDir)).artifactsDir || '.artifacts';
    cfg.toolCatalog = {
        path: relCatalog,
    ...
    if (!dryRun && cfgChanged)
        await writeJson(mcpConfigPath, cfg);
