...
export function resolveNpmCmd(): string {
...
export function codexAbs(): string {
  return join(process.env.APPDATA ?? "", "npm", "codex.cmd");
}

export function npxAbs(): string {
  return join(dirname(process.execPath), "npx.cmd");
}

export function resolveGeminiCmd(): string[] {
  ...
  return guesses.filter((guess) => {
    if (!guess.endsWith(".cmd")) return true;
    try {
...
export function gitCandidates(): string[] {
  ...
  if (process.platform === "win32") {
    ...
    for (const guess of guesses) {
      try {
        if (existsSync(guess)) list.push(guess);
      } catch {
        // ignore
      }
    }
...
export function sfCandidates(): string[] {
  const list: string[] = [];
  if (process.platform === "win32") {
    const pf = process.env["ProgramFiles"] || "C:/Program Files";
    const pf86 = process.env["ProgramFiles(x86)"] || "C:/Program Files (x86)";
