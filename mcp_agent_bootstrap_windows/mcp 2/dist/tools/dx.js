/**
 * dx.ts is the bridge between local tools and Salesforce CLI data plane: orgSoql/orgToolingSoql capture
 * query output, orgAssignPermissionSet applies permissions, and orgAgentTest runs agent utterances while
 * storing artifacts in predictable .artifacts/dx or .artifacts/agent folders.
...
function makeStableId(input, fallback) {
...
export async function orgSoql(repoDir, orgAlias, soql) {
...
export async function orgToolingSoql(repoDir, orgAlias, soql) {
    const cfg = await resolveProject(repoDir);
...
export async function orgAssignPermissionSet(repoDir, orgAlias, permissionSetName) {
    const cfg = await resolveProject(repoDir);
...
export async function orgAgentTest({ repoDir, orgAlias, ticketId, packPath, agentApiName, context, capture, utterances, mode, scope, maxTurns, waitMinutes, }) {
    const cfg = await resolveProject(repoDir);
