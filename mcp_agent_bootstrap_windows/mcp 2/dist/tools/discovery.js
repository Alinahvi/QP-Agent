/**
 * discovery.ts owns the discovery pipeline. Each function may hit the Salesforce org via sf CLI fallbacks
 * and always persists normalized artifacts locally:
 *   - orgDiscoverObject / orgDiscoverUsage / orgDiscoverPermissions: org-first, repo fallback, writes JSON.
 *   - discoveryPlan / discoveryBrief: derive plan + Markdown using the JSON artifacts.
 *   - discoveryRun / discoveryFanout helpers (imported elsewhere) stitch everything with diagnostics.
...
export async function orgDiscoverObject(repoDir, orgAlias, objectApiName) {
    ...
    const getTags = (xml, tag) => Array.from(xml.matchAll(new RegExp(`<${tag}>\\s*([\\s\\S]*?)\\s*<\\/${tag}>`, 'gi'))).map(m => m[1].trim());
    ...
    try {
        if (!(await sfAvailable(repoDir))) {
            throw new Error('sf CLI not available');
        }
        ...
        if (ent.records?.length)
            label = ent.records[0].Label || label;
        // FieldDefinition -> fields
        const fldRes = await toolingQuery(repoDir, orgAlias, `SELECT QualifiedApiName, DataType, Label, Length, Precision, Scale, IsCalculated, IsNillable, IsUnique, IsExternalId, RelationshipName, ReferenceTargetField, ReferenceTargetEntity 
       FROM FieldDefinition WHERE EntityDefinition.QualifiedApiName = '${objectApiName}'`);
        fields = (fldRes.records || []).map((r) => ({
        ...
        const vrRes = await toolingQuery(repoDir, orgAlias, `SELECT Active, Description, ErrorConditionFormula, ErrorDisplayField, ErrorMessage, ValidationName, EntityDefinition.QualifiedApiName 
       FROM ValidationRule WHERE EntityDefinition.QualifiedApiName = '${objectApiName}'`);
        validationRules = (vrRes.records || []).map((r) => ({
        ...
        // RecordType (Data API)
        const rtRes = await dataQuery(repoDir, orgAlias, `SELECT DeveloperName, Name, IsActive FROM RecordType WHERE SObjectType = '${objectApiName}'`);
        recordTypes = (rtRes.records || []).map((r) => ({ apiName: r.DeveloperName, label: r.Name, active: !!r.IsActive }));
        // ApexTrigger (Tooling)
        const trgRes = await toolingQuery(repoDir, orgAlias, `SELECT Name, Status FROM ApexTrigger WHERE TableEnumOrId = '${objectApiName}'`);
        triggers = (trgRes.records || []).map((r) => ({ name: r.Name, status: r.Status || 'Active' }));
    }
    catch {
...
export async function orgDiscoverUsage(repoDir, orgAlias, objectApiName) {
    const cfg = await resolveProject(repoDir);
    const outDir = path.join(repoDir, cfg.artifactsDir, 'discovery', objectApiName);
    await ensureDir(outDir);
    ...
    const orgFlows = [];
    const orgLwcs = [];
    const parseFlowMarkupForObject = (xml) => {
        const out = [];
        if (!xml)
            return out;
        const add = (block, element, action) => {
            const fields = Array.from(block.matchAll(/<field>\s*([^<]+)\s*<\/field>/gi)).map(m => m[1].trim());
            out.push({ element, action, fields });
        };
        const hasObj = (block) => new RegExp(`<sobject>\s*${objectApiName}\s*<\/sobject>|<object>\s*${objectApiName}\s*<\/object>`, 'i').test(block);
        for (const m of xml.matchAll(/<recordCreates>[\s\S]*?<\/recordCreates>/gi)) {
            const b = m[0];
            if (hasObj(b))
                add(b, 'recordCreates', 'create');
        }
        for (const m of xml.matchAll(/<recordUpdates>[\s\S]*?<\/recordUpdates>/gi)) {
            const b = m[0];
            if (hasObj(b))
                add(b, 'recordUpdates', 'update');
        }
        for (const m of xml.matchAll(/<recordLookups>[\s\S]*?<\/recordLookups>/gi)) {
            const b = m[0];
            if (hasObj(b))
                add(b, 'recordLookups', 'lookup');
        }
        for (const m of xml.matchAll(/<recordDeletes>[\s\S]*?<\/recordDeletes>/gi)) {
            const b = m[0];
            if (hasObj(b))
                add(b, 'recordDeletes', 'delete');
        }
        // generic element blocks
        for (const m of xml.matchAll(/<elements>[\s\S]*?<\/elements>/gi)) {
            const b = m[0];
            if (hasObj(b))
                add(b, 'elements', 'unknown');
        }
        return out;
    };
    try {
        if (await sfAvailable(repoDir)) {
            ...
            const ac = await toolingQuery(repoDir, orgAlias, `SELECT Name, Status, ApiVersion FROM ApexClass WHERE Body LIKE '%${like}%'`);
            ...
            const fl = await toolingQuery(repoDir, orgAlias, `SELECT FullName, ActiveVersion.VersionNumber, Markup FROM Flow WHERE Markup LIKE '%${like}%'`);
            ...
            const lw = await toolingQuery(repoDir, orgAlias, `SELECT FilePath, Format FROM LightningComponentResource WHERE Source LIKE '%${like}%'`);
...
export async function orgDiscoverPermissions(repoDir, orgAlias, objectApiName) {
    const cfg = await resolveProject(repoDir);
    const outDir = path.join(repoDir, cfg.artifactsDir, 'discovery', objectApiName);
    await ensureDir(outDir);
    const crud = { create: false, read: false, update: false, delete: false };
    const fieldPermissions = {};
    const recordTypeVisibility = [];
    const customPermissions = [];
    const sources = { profiles: [], permissionSets: [] };
    try {
        const opq = await dataQuery(repoDir, orgAlias, `SELECT SObjectType, PermissionsRead, PermissionsCreate, PermissionsEdit, PermissionsDelete, ParentId FROM ObjectPermissions WHERE SObjectType = '${objectApiName}'`);
        ...
        const fpq = await dataQuery(repoDir, orgAlias, `SELECT Field, PermissionsRead, PermissionsEdit, ParentId FROM FieldPermissions WHERE Field LIKE '${objectApiName}.%'`);
        ...
        const rtv = await dataQuery(repoDir, orgAlias, `SELECT Profile.Name, RecordType.DeveloperName, Visible FROM ProfileRecordTypeVisibility WHERE RecordType.SObjectType = '${objectApiName}'`);
        ...
        const cp = await dataQuery(repoDir, orgAlias, `SELECT DeveloperName, Label FROM CustomPermission WHERE DeveloperName LIKE '%${objBase}%' OR Label LIKE '%${objBase}%'`);
        for (const r of cp.records || []) {
            const name = r.DeveloperName || r.Label;
            if (name && !customPermissions.includes(name))
                customPermissions.push(name);
        }
    }
    catch {
    ...
    if (!crud.read && !crud.create && !crud.update && !crud.delete && Object.keys(fieldPermissions).length === 0) {
        ...
        for (const file of allFiles) {
            ...
            if (isProfile)
                sources.profiles.push(path.basename(file));
            else
                sources.permissionSets.push(path.basename(file));
            ...
            for (const block of types) {
                ...
                if (!objMatch)
                    continue;
                if (objMatch[1].trim() !== objectApiName)
                    continue;
                const flag = (tag) => new RegExp(`<allow${tag}>\\s*true\\s*<\\/allow${tag}>`, 'i').test(block);
            ...
            for (const block of fblocks) {
                ...
                if (!full.startsWith(objectApiName + '.'))
                    continue;
            ...
            for (const block of rblocks) {
                ...
                if (!rt.startsWith(objectApiName + '.'))
                    continue;
...
export async function discoveryRun(repoDir, orgAlias, objectApiName, branchRef = 'origin/main') {
    const cfg = await resolveProject(repoDir);
    const outDir = path.join(repoDir, cfg.artifactsDir, 'discovery', objectApiName);
    await ensureDir(outDir);
    ...
    const briefPath = await (async () => {
        try {
            return await (await import('./brief.js')).discoveryBrief(repoDir, objectApiName, branchRef);
        }
        catch {
            return '';
        }
...
export async function discoveryPlan(repoDir, objectApiName) {
    const cfg = await resolveProject(repoDir);
