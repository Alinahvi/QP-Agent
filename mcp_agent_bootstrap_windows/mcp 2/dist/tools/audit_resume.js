/**
 * audit_resume.ts keeps work-in-progress context purely in the local repo: auditActionLayer inspects
 * generated Apex for single-output conformance, while resumeContext writes next-steps memos that stitch
 * together plan artifacts for future agents.
 */ import * as path from 'path';
import fs from 'fs-extra';
import { resolveProject } from '../lib/resolver.js';
export async function auditActionLayer(repoDir, objectApiName, apply = false) {
    const classesDir = path.join(repoDir, 'force-app', 'main', 'default', 'classes');
    const files = await fs.readdir(classesDir);
    const handler = files.find((f) => f.includes(objectApiName.replace('__c', '')) && f.endsWith('_Handler.cls'));
    const report = { objectApiName, handlerFound: Boolean(handler), singleOutput: null, changes: [] };
...
export async function resumeContext(repoDir, objectApiName) {
