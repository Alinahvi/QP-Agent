...
function parseLegacyKeyValues(text) {
    const res = {};
    ...
    if (utterancesRaw) {
        try {
            ...
            if (Array.isArray(parsed))
                utterances = parsed.map((u) => String(u));
        }
        catch {
...
export async function ticketsCollect(repoDir, opts) {
    ...
    for (const abs of Array.from(fileSet)) {
        ...
        if (!includeDerived && isDerived)
            continue;
        if (!includeBacklog && !isDerived)
            continue;
        let text = '';
        try {
            text = await (await import('fs')).promises.readFile(abs, 'utf8');
        }
        catch {
            continue;
        }
...
export async function planningFanout(repoDir, orgAlias, opts) {
    const cfg = await resolveProject(repoDir);
    const maxC = Math.min(5, Math.max(1, opts.maxConcurrency ?? 4));
    const lim = pLimit(maxC);
    const results = [];
    const stale = opts.staleHours ?? cfg?.planning?.staleHours ?? 24;
    await Promise.all((opts.objects || []).map((objectApiName) => lim(async () => {
        const run = { objectApiName, startedAt: new Date().toISOString() };
        try {
...
export async function ticketsFanout(repoDir, orgAlias, opts) {
    const cfg = await resolveProject(repoDir);
    ...
    const lim = pLimit(maxC);
    const results = [];
    const runOne = async (ticket) => {
        const startedAt = new Date().toISOString();
        const steps = [];
        const record = { id: ticket.id, path: ticket.path, object: ticket.objectApiName, startedAt, steps };
        try {
            if (ticket.objectApiName) {
                steps.push({ name: 'planning:resume', status: 'running' });
                const pr = await planningResume(repoDir, orgAlias, ticket.objectApiName, cfg?.planning?.staleHours ?? 24);
...
export async function auditActionLayerFanout(repoDir, opts) {
