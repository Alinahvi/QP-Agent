...
function discDir(repoDir, artifactsDir, obj) {
...
export async function centralView(repoDir) {
    const cfg = await resolveProject(repoDir);
    const roots = cfg.searchRoots || ['force-app/main/default/classes'];
    const inv = { handlers: [], services: [], invocable: [] };
    for (const r of roots) {
        const base = path.join(repoDir, r);
        let files = [];
        try {
            files = (await fsp.readdir(base)).filter(f => f.endsWith('.cls'));
        }
        catch {
            continue;
        }
        for (const f of files) {
            ...
            let src = '';
            try {
                src = await fsp.readFile(p, 'utf8');
            }
            catch { }
            const isHandler = /class\s+\w+Handler\b/.test(src);
            const isService = /class\s+\w+Service\b/.test(src);
            if (isHandler)
                inv.handlers.push({ file: p.replace(repoDir + path.sep, '') });
            if (isService)
                inv.services.push({ file: p.replace(repoDir + path.sep, '') });
    ...
    const lines = ['# Centralized Action View', '',
        '## Handlers', ...(inv.handlers.length ? inv.handlers.map((h) => `- ${h.file}`) : ['(none)']),
        '', '## Services', ...(inv.services.length ? inv.services.map((s) => `- ${s.file}`) : ['(none)']),
        '', '## Invocable Methods', ...(inv.invocable.length ? inv.invocable.map((i) => `- ${i.label || '(no label)'} - ${i.file}`) : ['(none)'])];
    await writeText(out, lines.join('\n'));
    return { ok: true, out };
}
function normalizeObjectsArg(arg) {
    if (!arg)
        return undefined;
    if (Array.isArray(arg))
        return arg.map((s) => String(s).replace(/^['"]|['"]$/g, ''));
    if (typeof arg === 'string') {
        const s = arg.trim();
        try {
            const arr = JSON.parse(s);
            if (Array.isArray(arr))
                return arr.map((v) => String(v).replace(/^['"]|['"]$/g, ''));
        }
        catch { }
...
export async function impactMatrix(repoDir, objects) {
    const cfg = await resolveProject(repoDir);
    const base = path.join(repoDir, cfg.artifactsDir || '.artifacts', 'discovery');
    const normalized = normalizeObjectsArg(objects);
    let objs = [];
    try {
        objs = normalized && normalized.length ? normalized : (await fsp.readdir(base)).filter(n => !n.startsWith('.'));
    }
    catch {
        objs = normalized || [];
    }
    ...
    for (const obj of objs) {
        ...
        let usage = {}, meta = {}, perms = {};
        try {
            usage = await readJson(usageP);
        }
        catch { }
        try {
            meta = await readJson(objP);
        }
        catch { }
        try {
            perms = await readJson(permsP);
        }
        catch { }
        rows.push({
            object: obj,
            ...
            activeVRs: (meta.validationRules || []).filter((v) => v.active).length || 0,
            crud: perms.crud || {},
        });
    }
...
export async function proposePlans(repoDir, objects) {
    const cfg = await resolveProject(repoDir);
    ...
    for (const obj of objs) {
        ...
        const allFields = Array.isArray(meta.fields) ? meta.fields.map((f) => f.fullName || f.name || '').filter(Boolean) : [];
        ...
        const plan = {
            ...
            dto: {
                detailFields: detail,
                upsertFields: upsert,
                search: { queryFields: detail.filter((f) => !['Id', 'RecordTypeId'].includes(f)), defaultOrderBy: 'CreatedDate DESC', limit: 200 }
            },
...
export async function seedTickets(repoDir) {
    const cfg = await resolveProject(repoDir);
    const plansDir = path.join(repoDir, cfg.artifactsDir || '.artifacts', 'plan');
    ...
    const mk = (obj, id, title, runAgent = false) => `---\nid: ${obj}-${id}\npriority: P1\nobject: ${obj}\nsuite:\n  - FR_Smoke\nallowlist: []\nrunAgent: ${runAgent}\n---\n# ${title}\n\n**DoD**\n- Plan v2 present\n- Brief refreshed\n- CRUD/FLS enforced\n- Tests green\n`;
    const results = [];
    for (const f of files) {
...
export async function normalizeDiscovery(repoDir, sourceDir) {
