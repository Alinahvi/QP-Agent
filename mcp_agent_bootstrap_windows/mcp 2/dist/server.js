...
async function startMcp() {
    ...
    const defs = toolDefs();
    for (const t of defs) {
        server.tool(t.name, t.description, t.schema, { title: t.title }, async (args) => {
            const repoDir = args?.repoDir || defaultRepoDir();
            const result = await t.handler({ ...(args ?? {}), repoDir });
...
async function startCli() {
    ...
    if (cmd === 'list') {
        const minimal = defs.map((d) => ({
            ...
            inputs: Object.keys(d.schema?.shape || {}),
        }));
