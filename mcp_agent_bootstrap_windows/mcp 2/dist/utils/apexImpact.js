...
function normalizeRelativePath(repoDir, filePath) {
    const rel = relative(repoDir, filePath);
    return rel.replace(/\\/g, "/");
}
function isWithinRoots(repoDir, filePath, roots) {
    if (!roots.length)
        return true;
...
function detectApexClassName(src) {
    const match = src.match(/\b(class|interface|enum)\s+([A-Za-z0-9_]+)/);
    return match ? match[2] : null;
}
function isApexTestClass(src, maybeFilename) {
    if (/\b@IsTest\b/i.test(src) || /\btestMethod\b/i.test(src))
        return true;
    if (maybeFilename) {
        const name = basename(maybeFilename, extname(maybeFilename));
        if (/test(s)?$/i.test(name) || /^test/i.test(name) || /_tests?$/i.test(name))
            return true;
    }
    return false;
}
function deriveLikelyTestNames(base) {
    return [
...
async function listFiles(root, exts, max, searchRoots) {
    const results = [];
...
async function runGitDiff(repoDir, fromRef, toRef) {
    let last = null;
...
async function gatherGitChangedClasses(repoDir, fromRef, toRef, roots) {
    const diffResult = await runGitDiff(repoDir, fromRef, toRef);
    const changedClasses = new Set();
    const changedTestClasses = new Set();
    ...
    const workers = Array.from({ length: limit }, async () => {
        while (true) {
            ...
            if (!name)
                continue;
            if (isApexTestClass(src, file))
                changedTestClasses.add(name);
            else
                changedClasses.add(name);
        }
    });
    await Promise.all(workers);
    return { changedClasses, changedTestClasses };
}
async function loadManifestClasses(manifestPath) {
...
async function buildTestMap(files) {
    const map = new Map();
    if (!files.length)
        return map;
...
export async function computeImpactedApexTests(opts) {
    ...
    if (changedNames.length) {
        ...
        for (const { name, src } of testMap.values()) {
            if (patterns.some((regex) => regex.test(src)))
                referenced.add(name);
        }
    }
    ...
    for (const name of changedClasses) {
        for (const guess of deriveLikelyTestNames(name)) {
            for (const candidate of testMap.keys()) {
                if (candidate.toLowerCase() === guess.toLowerCase())
                    heuristic.add(candidate);
            }
        }
    }
    const combined = new Set();
    for (const name of changedTestClasses)
        combined.add(name);
