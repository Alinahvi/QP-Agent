...
export async function apexRunSpecified(repoDir, org, tests, waitSeconds = 2400) {
...
export async function apexRunSuites(repoDir, org, suites, waitSeconds = 2400) {
...
export async function assignPermissionSet(repoDir, org, permissionSetName) {
    return trySh('sf', [
        'org',
        'permset',
        'assign',
...
export async function runAgentTest(repoDir, org, opts) {
    ...
    if (hasEinstein && opts.utterances && opts.utterances.length) {
        ...
        if (opts.definitionName)
            args.push('--definition-name', opts.definitionName);
        if (opts.agentApiName)
            args.push('--agent-api-name', opts.agentApiName);
        if (opts.scope)
            args.push('--scope', opts.scope);
        if (opts.mode)
            args.push('--mode', opts.mode);
        if (opts.capture && opts.capture.length)
            args.push('--capture', opts.capture.join(','));
...
export async function toolingQuery(repoDir, org, soql) {
    let last = '';
    for (let i = 0; i < 3; i++) {
