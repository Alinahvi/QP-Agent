/**
 * scaffold.ts generates Apex scaffolding from plan data: actionlayerScaffold renders Mustache templates,
 * phase2RequirementsStub/scaffoldPhase2ForObject reuse Audience examples, and everything operates on the
 * local filesystem while optionally reading plan/requirements artifacts.
...
type TemplateSet = 'generated'|'phase2';

type Phase2Requirements = {
  scope?: { primaryObject?: string };
  actionLayer?: {
    invocableMethod?: { className?: string; methodLabel?: string; methodName?: string; singleOutputEnvelope?: { contractName?: string; version?: string } };
    actions?: any[];
    inputs?: any[];
    outputData?: any;
  };
  ...
  objects?: string[];
  related?: {
    member?: { objectApiName?: string; parentFieldApiName?: string; relationshipNameOnParent?: string };
    checklist?: { objectApiName?: string };
    assignedLearning?: { objectApiName?: string };
  };
  recordTypes?: {
    create?: { developerName?: string; label?: string };
  };
};
...
function toObjectKey(objectApiName: string) {
  return String(objectApiName || '').replace('__c','');
}

function classBase(objectApiName: string) {
...
function applyPhase2Replacements(src: string, objectApiName: string, req?: Phase2Requirements) {
...
export async function phase2RequirementsStub(
  repoDir: string,
  ...
  let rtDev: string | undefined; let rtLabel: string | undefined;
  ...
  try {
    ...
    const manual = rts.find((r: any) => /manual/i.test(r?.label || '')) || rts[0];
    if (manual) { rtDev = manual.apiName || manual.developerName; rtLabel = manual.label; }
  } catch {}

  const yamlObj: Phase2Requirements = {
...
export async function scaffoldPhase2ForObject(
  repoDir: string,
  objectApiName: string,
  mode: Mode = 'apply',
  planPath?: string,
  ...
  for (const p of defaultPlanCandidates) { if (await pathExists(p)) { chosenPlan = p; break; } }
  if (!chosenPlan) throw new Error(`Plan file not found for ${obj}. Provide --planPath or generate plan.v2 first.`);
