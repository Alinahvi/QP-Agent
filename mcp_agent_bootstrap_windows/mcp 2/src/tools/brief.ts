/**
 * discoveryBrief transforms discovery + plan outputs into a Markdown brief (local only) so collaborators
 * can review object context alongside CRUD/FLS guardrails before executing scaffolds.
...
export async function discoveryBrief(repoDir: string, objectApiName: string, branchRef='origin/main') {
  ...
  const view = {
    ...
    recordTypes: (obj.recordTypes||[]).map((r:any)=>r.apiName).join(', ') || '-',
    keyFields: (plan.dto?.detailFields||[]).join(', ') || '-',
    validationRules: obj.validationRules || [],
    triggers: (obj.triggers||[]).map((t:any)=>t.name).join(', ') || '-',
    ...
    whatToAdd: (plan.todo?.whatToAdd||[]).map((s:string)=>({item:s})),
    niceToHaves: (plan.todo?.niceToHaves||[]).map((s:string)=>({item:s})),
    impacted: (plan.todo?.impacted||[]).map((s:string)=>({item:s})),
    rollout: (plan.todo?.rollout||[]).map((s:string)=>({item:s})),
    tests: (plan.todo?.tests||[]).map((s:string)=>({item:s})),
    risks: (plan.todo?.risks||[]).map((s:string)=>({item:s})),
    usageApex: (usage.apex || []).map((a:any)=>a.name),
    usageFlows: (usage.flows || []).map((f:any)=>f.fullName||f.label),
    usageLwc: (usage.lwcs || []).map((l:any)=>l.component||l.resource),
...
`;
  ...
  (view as any).crudJson = JSON.stringify(view.crud || {}, null, 0);
