...
export async function schemaIndex(repoDir: string, staleHours = 12) {
  ...
  if (idxMtime && idxMtime > ttlCutoff) {
    const idx = await readJson<SchemaIndex>(outPath).catch(() => null);
    if (idx) return { outPath, objectsCount: Object.keys(idx.objects||{}).length, componentsCount: Object.keys(idx.components||{}).length, cached: true };
  }

  const lwcRoot = path.join(repoDir, 'force-app', 'main', 'default', 'lwc');
  const compDirs = await fg('*/', { cwd: lwcRoot, onlyDirectories: true }).catch(() => [] as string[]);

  const components: SchemaIndex['components'] = {};
  const objects: SchemaIndex['objects'] = {};

  for (const comp of compDirs) {
    const compPath = path.join(lwcRoot, comp);
    const files = await fg(['**/*.js','**/*.ts','**/*.html'], { cwd: compPath, onlyFiles: true }).catch(()=>[] as string[]);
    ...
    for (const rel of files) {
      ...
      let src = '';
      try { src = await fsp.readFile(abs, 'utf8'); } catch { continue; }
    ...
    for (const obj of objectsArr) {
      ...
      entry.count++;
      if (!entry.components.includes(comp.replace(/\/$/, ''))) entry.components.push(comp.replace(/\/$/, ''));
  ...
  const index: SchemaIndex = { generatedAt: new Date().toISOString(), components, objects };
