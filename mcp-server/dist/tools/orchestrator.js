...
import { startFromObject, startFromLwc } from './entrypoints.js';
async function scanProjectForCandidates(repoDir, searchRoots) {
    const { promises: fsp } = await import('fs');
    const setObjects = new Set();
    const setLwc = new Set();
...
export async function discoveryFanout(repoDir, orgAlias, opts) {
    const cfg = await resolveProject(repoDir);
    const maxC = Math.min(5, Math.max(1, opts.maxConcurrency ?? 4));
    const lim = pLimit(maxC);
    const startedAt = new Date().toISOString();
    const results = [];
    const runForObject = async (obj) => {
        const r = { objectApiName: obj, startedAt: new Date().toISOString() };
        try {
    ...
    const runForLwc = async (comp) => {
        const r = { lwc: comp, startedAt: new Date().toISOString() };
        try {
    ...
    for (const o of opts.objects || [])
        tasks.push(lim(() => runForObject(o)));
    for (const c of opts.lwcComponents || [])
        tasks.push(lim(() => runForLwc(c)));
...
export async function discoveryFromProject(repoDir, orgAlias, staleHours, maxConcurrency, doPlan, doBrief) {
    const cfg = await resolveProject(repoDir);
