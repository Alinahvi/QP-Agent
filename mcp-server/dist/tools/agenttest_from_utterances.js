...
async function writeSpecYaml(repoDir, agentApiName, testApiName, utterances, nonDestructive) {
    ...
    let defaultActions = [];
    try {
        const plannerPath = path.join(repoDir, 'force-app', 'main', 'default', 'genAiPlannerBundles', agentApiName, `${agentApiName}.genAiPlannerBundle`);
        const xml = await fs.readFile(plannerPath, 'utf8');
        const topicMatch = xml.match(/<genAiPluginName>([^<]+)<\/genAiPluginName>/);
        if (topicMatch)
            defaultTopic = topicMatch[1];
...
export async function agentTestFromUtterances(repoDir, orgAlias, agentApiName, testApiName, utterances, waitMinutes = 5, nonDestructive = true) {
    if (!agentApiName)
        throw new Error('agentApiName is required');
    if (!testApiName)
        throw new Error('testApiName is required');
    if (!utterances || !utterances.length)
        throw new Error('utterances must be a non-empty array');
