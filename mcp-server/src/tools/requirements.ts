/**
 * requirements.ts handles the requirements ingestion funnel entirely locally: validate/gapcheck enforce
 * schema, ingest_docs/dissect/assemble convert freeform notes into canonical YAML, and helpers extract
 * object hints for downstream planning.
...
function topicFrom(file: string) {
...
function classifyErrors(errors: ErrorObject[] | null | undefined) {
  const list: Array<{ severity: 'P0' | 'P1'; message: string; path: string }> = [];
  for (const err of errors || []) {
    const severity: 'P0' | 'P1' = err.keyword === 'required' ? 'P0' : 'P1';
    const pointer = (err as any).instancePath || (err as any).schemaPath || '';
    list.push({ severity, message: `${err.keyword}: ${err.message ?? 'invalid'}`, path: pointer });
  }
  return list;
}

async function ensurePlanningDir(repoDir: string, topic: string, sub?: string) {
...
export async function requirementsValidate(repoDir: string, file: string) {
  ...
  const issues = classifyErrors(validateRequirements.errors as any);
...
export async function requirementsGapcheck(repoDir: string, topic: string, file?: string) {
  ...
  const gaps: Array<{ severity: 'P0' | 'P1'; note: string }> = [];
  if (!data?.businessRules) gaps.push({ severity: 'P1', note: 'Add businessRules defaults and edge-case guidance.' });
  if (!data?.tests?.scenarios || data.tests.scenarios.length === 0) gaps.push({ severity: 'P0', note: 'Define at least one acceptance scenario under tests.scenarios.' });
  if (!data?.dependencies) gaps.push({ severity: 'P1', note: 'List required dependencies (apexLibs, permissionSets, recordTypes).' });
  if (!data?.actionLayer?.inputs || data.actionLayer.inputs.length === 0) gaps.push({ severity: 'P1', note: 'Document expected action inputs (key/type/required).' });
...
export async function requirementsIngestDocs(repoDir: string, topic: string, sources: string[]) {
  ...
  for (const src of sources) {
    ...
    let current = { title: 'root', content: [] as string[] };
    for (const line of lines) {
      const heading = line.match(/^#{1,6}\s+(.*)$/);
      if (heading) {
        if (current.content.length) segments.push({ title: current.title, content: current.content.join('\n').trim() });
...
export async function requirementsDissect(repoDir: string, topic: string) {
  ...
  const buckets = { summary: [] as string[], goals: [] as string[], requirements: [] as string[], constraints: [] as string[], acceptance: [] as string[] };
  for (const entry of entries) {
    for (const section of entry.sections || []) {
      const title = String(section.title || '').toLowerCase();
      const content: string = section.content || '';
      if (!content.trim()) continue;
      if (title.includes('goal')) buckets.goals.push(content);
      else if (title.includes('constraint') || title.includes('risk')) buckets.constraints.push(content);
      else if (title.includes('acceptance') || title.includes('test')) buckets.acceptance.push(content);
      else if (title.includes('require') || title.includes('need')) buckets.requirements.push(content);
      else if (title.includes('summary') || title.includes('overview')) buckets.summary.push(content);
      else buckets.requirements.push(content);
    }
  }
...
function extractObjects(chunks: string[]) {
...
export async function requirementsAssemble(repoDir: string, topic: string, dissectPath?: string) {
  const { base } = await ensurePlanningDir(repoDir, topic);
  ...
  const buckets = (dissect as any).buckets || { summary: [], goals: [], requirements: [], constraints: [], acceptance: [] };
  const objects = extractObjects([...(buckets.requirements as string[]), ...(buckets.summary as string[])]);
  const primary = objects[0] || `${topic}__c`;
  const actionCandidates = (buckets.requirements as string[])
    .flatMap((item: string) => item.split(/\n+/))
    .map((line: string) => line.trim())
    .filter(Boolean)
    .slice(0, 8);
  const actions = actionCandidates.map((line: string, idx: number) => ({
